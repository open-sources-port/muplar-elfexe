...
/* WXN=0 explicitly: this is what the test is checking */
                         ...
                         vcpu_exit_t *out)
{
    for (int iter = 0; iter < 10000; iter++) {
        ...
        fprintf(stderr, "  unexpected exit reason=0x%x syndrome=0x%llx\n",
                vexit->reason, (unsigned long long) vexit->exception.syndrome);
        out->reason = HVF_EXIT_ERROR;
        return -1;
    }
...
static void print_bad_exception(const vcpu_exit_t *ex)
{
    ...
    fprintf(stderr, "    FAR=0x%llx  ELR=0x%llx  SPSR=0x%llx\n",
            (unsigned long long) ex->far, (unsigned long long) ex->elr,
...
static int test1_rwx_block(void)
{
     ...
     *
     * The instructions the test wants to write to the RWX page:
...
#define _(insn) a64_##insn
    {
        uint32_t mov_x0_42 = _(movz(0, 42)); /* 0xD2800540 */
        uint32_t svc_0 = _(svc0());          /* 0xD4000001 */

        uint32_t code[] = {
            /* Load "mov x0, #42" encoding into x1 */
            _(movz(1, (uint16_t) (mov_x0_42 & 0xFFFF))),
            _(movk_lsl16(1, (uint16_t) ((mov_x0_42 >> 16) & 0xFFFF))),

            /* Load RWX_BLOCK address into x2 */
            _(movz(2, (uint16_t) (RWX_BLOCK & 0xFFFF))),
            _(movk_lsl16(2, (uint16_t) ((RWX_BLOCK >> 16) & 0xFFFF))),

            /* Write "mov x0, #42" to RWX page */
            _(str_w_imm(1, 2, 0)),

            /* Load "svc #0" encoding into x1 */
            _(movz(1, (uint16_t) (svc_0 & 0xFFFF))),
            _(movk_lsl16(1, (uint16_t) ((svc_0 >> 16) & 0xFFFF))),
        ...
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_TEST1, code,
                      ARRAY_SIZE(code));
    }
#undef _
    ...
    if (setup_vcpu(vcpu, ttbr0, GUEST_CODE + CODE_TEST1, SP_EL0, SP_EL1,
                   SHIM_BASE) < 0) {
    ...
    if (ex.reason == HVF_EXIT_HVC9) {
    ...
    } else if (ex.reason == HVF_EXIT_HVC5) {
    ...
    } else if (ex.reason == HVF_EXIT_HVC2) {
        ...
        if (ec == 0x20) {
            printf(
                "    " YELLOW
                "Instruction abort: W^X blocks execution on writable page" RESET
                "\n");
        } else if (ec == 0x24) {
            printf("    " YELLOW
                   "Data abort: W^X blocks write to executable page" RESET
                   "\n");
        }
        result = -1;
...
#define _(insn) a64_##insn
    {
        uint32_t mov_x0_42 = _(movz(0, 42)), svc_0 = _(svc0());

        uint32_t code[] = {
            _(movz(1, (uint16_t) (mov_x0_42 & 0xFFFF))),
            _(movk_lsl16(1, (uint16_t) ((mov_x0_42 >> 16) & 0xFFFF))),

            _(movz(2, (uint16_t) (RWX_PAGE_ADDR & 0xFFFF))),
            _(movk_lsl16(2, (uint16_t) ((RWX_PAGE_ADDR >> 16) & 0xFFFF))),
        ...
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_TEST2, code,
                      ARRAY_SIZE(code));
    }
#undef _
    ...
    if (setup_vcpu(vcpu, ttbr0, GUEST_CODE + CODE_TEST2, SP_EL0, SP_EL1,
                   SHIM_BASE) < 0) {
        hv_vcpu_destroy(vcpu);
        vm_destroy(&vm);
        return -1;
...
static int test3_baseline_rx(void)
{
    ...
    /* Baseline guest code: mov x0, #99; mov x8, #0; svc #0 */
#define _(insn) a64_##insn
    uint32_t code[] = {
        _(movz(0, 99)),
        _(movz(8, 0)), /* syscall number is irrelevant; X0 is the assertion */
        _(svc0())};
#undef _
    hvf_emit_code(vm.host_base, GUEST_CODE + CODE_TEST3, code,
                  ARRAY_SIZE(code));
    ...
    if (setup_vcpu(vcpu, ttbr0, GUEST_CODE + CODE_TEST3, SP_EL0, SP_EL1,
                   SHIM_BASE) < 0) {
        hv_vcpu_destroy(vcpu);
        vm_destroy(&vm);
        return -1;
    ...
    int result = -1;
    if (ex.reason == HVF_EXIT_HVC5 && ex.x0 == 99) {
        result = 0;
    } else if (ex.reason == HVF_EXIT_HVC2) {
        print_bad_exception(&ex);
        result = -1;
...
#define _(insn) a64_##insn
    {
        uint32_t code[] = {
            ...
            _(movz(8, 0)),
            _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_TEST4, code,
                      ARRAY_SIZE(code));
    }
#undef _
    ...
    if (setup_vcpu(vcpu, ttbr0, GUEST_CODE + CODE_TEST4, SP_EL0, SP_EL1,
                   SHIM_BASE) < 0) {
        hv_vcpu_destroy(vcpu);
        vm_destroy(&vm);
        return -1;
    ...
    int result = -1;
    if (ex.reason == HVF_EXIT_HVC5 && ex.x0 == 0x77) {
        result = 0;
    } else if (ex.reason == HVF_EXIT_HVC2) {
        print_bad_exception(&ex);
        result = -1;
...
int main(void)
{
    ...
    printf("=========================================================\n\n");
    printf(
        "Question: Does HVF allow simultaneous RWX page table entries\n"
    ...
    if (pass == ntests) {
        printf(
            ...
            "when SCTLR_EL1.WXN=0. Self-modifying code works without W^X "
            "toggling.\n" RESET);
    } else if (pass >= 2 && fail > 0) {
        printf(
            YELLOW
            "CONCLUSION: Apple HVF enforces W^X at stage-2 (hypervisor level)\n"
            "regardless of stage-1 page table permissions and SCTLR.WXN.\n"
            "RWX page table entries are silently restricted. The W^X "
            "demand-toggle\n"
            "mechanism (HVC #9) in the shim is REQUIRED for JIT use "
            "cases.\n" RESET);
    } else {
        printf(RED
               "CONCLUSION: Unexpected results -- check test output "
               "above.\n" RESET);
    }
